...
def timeDep(u,T,omega,Nt):
	'''Apply time dependence to a function.
	If field is real, same as multiplying by cos(2piwt).'''

	unew = np.zeros(u.shape+(Nt,))

	for ti in range(0,Nt):
		unew[:,:,ti] = u[:,:] * np.cos(2.*np.pi*omega*T[ti])
...
# timeAverage1D
def timeAverage1D(u,T,Nt):
# Use to calculate the time average of 1D input.
...
def zonalMean(u,x):
	...
	dx = x[1] - x[0]
	u_xav = np.trapz(u_new,x,dx,axis=1)

	return u_xav

#====================================================

def domainInt(u,x,dx,y,dy):
	"Return domain integral of field u."

	return np.trapz(np.trapz(extend(u),x,dx,axis=1),y,dy)

#====================================================

def selectModes(utilde,vtilde,etatilde,klim,smallscale,N):
	'''Set contribution from modes above/below a given wavenumber to zero.'''

	zero = np.zeros(N)
	# Retain small-scale terms...
	if smallscale:
		for ii in range(0,klim+1):
			utilde[:,ii] = zero 
			vtilde[:,ii] = zero
			etatilde[:,ii] = zero 
		for ii in range(N-klim,N):
			utilde[:,ii] = zero
			vtilde[:,ii] = zero
			etatilde[:,ii] = zero
	# ...or large-scale terms
	else:
		for ii in range(klim+1,N-klim):
			utilde[:,ii] = zero
			vtilde[:,ii] = zero
			etatilde[:,ii] = zero
		
	
	return utilde,vtilde,etatilde
		

#====================================================

def twoMode(utilde,vtilde,x,N):
	"Look at interaction between two wavenumbers. Can show zero zonal-mean interaction."
	...
	u1 = np.real(np.fft.ifft(u1,axis=1) / dx)
	u2 = np.real(np.fft.ifft(u2,axis=1) / dx)

	u0 = zonalMean(u1*u2,x)
	u11 = zonalMean(u1*u1,x)
